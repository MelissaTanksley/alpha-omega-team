...
Make it academic yet accessible, similar to BIOLA University level.`,
  ...
  return (
    ...
    >
      <Card className="bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-50 border-amber-200/50 shadow-xl overflow-hidden relative">
        <div className="absolute top-0 right-0 w-64 h-64 bg-gradient-to-br from-amber-200/20 to-transparent rounded-full -translate-y-32 translate-x-32" />
        ...
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-lg font-semibold text-amber-900">Today's Verse</CardTitle>
                <p className="text-sm text-amber-700/70">{progress.current_book} {progress.current_chapter}:{progress.current_verse}</p>
              </div>
            </div>
        ...
        <CardContent className="pt-4">
          {verse && (
            <div className="space-y-6">
              <blockquote className="text-xl md:text-2xl font-serif text-amber-950 leading-relaxed italic border-l-4 border-amber-400 pl-6 py-2">
                "{verse.verse_text}"
              </blockquote>
              ...
              {hebrewGreek && (
                <motion.div
                  ...
                  className="mt-6 p-6 bg-white/70 rounded-xl border border-amber-200 space-y-4"
                >
                    ...
                    {hebrewGreek.key_words?.map((word, idx) => (
                      <div key={idx} className="p-4 bg-amber-50 rounded-lg">
                        <div className="flex items-baseline gap-3 mb-2">
                          ...
                          <span className="text-sm text-amber-600">({word.transliteration})</span>
                          <span className="text-sm font-medium">"{word.word}"</span>
                        </div>
                        <p className="text-sm text-amber-900/80">{word.meaning}</p>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="space-y-3">
                    <div>
                      <h5 className="font-medium text-amber-900 mb-1">Theological Significance</h5>
                      <p className="text-sm text-amber-800/80">{hebrewGreek.theological_significance}</p>
                    </div>
                    <div>
                      <h5 className="font-medium text-amber-900 mb-1">Historical Context</h5>
                      <p className="text-sm text-amber-800/80">{hebrewGreek.historical_context}</p>
                    </div>
                  </div>
                </motion.div>
              )}
